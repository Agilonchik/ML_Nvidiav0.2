...
# ==========================================
def spatial_aware_loss(num_classes, class_weights=None, gamma=2.0, 
                       focal_weight=5.0, dice_weight=6.0, area_weight=2.0, 
                       entropy_weight=0.05, tv_weight=5e-6):
    ...
    def loss_fn(y_true, y_pred, return_components=False):
        ...
        focal_mod = tf.pow(1. - y_pred, gamma)
        l_focal = tf.reduce_mean(tf.reduce_sum(ce * focal_mod * class_weights, axis=-1)) * focal_weight
        ...
        if return_components:
            return {"focal": l_focal, "dice": l_dice, "entropy": l_entropy, "area": l_area, "tv": l_tv}
...
class ModelBuilder:
    ...
    def build(self, trainable_encoder=False) -> tf.keras.Model:
        self.logger.info(f"🚀 Building U-Net with MobileNetV2. Encoder Trainable: {trainable_encoder}")
        ...
        # 5. Выходной слой
        outputs = layers.Conv2D(self.num_classes, 1, activation='softmax', dtype='float32')(u5)
        ...
        for idx in [6, 11]:  # Проверь индексы "маляров"
            if self.num_classes > idx: weights[idx] = 0.5 
        for idx in [1, 2, 9, 10]:  # Близнецы
            if self.num_classes > idx: weights[idx] = 4.0 

        model.compile(
            optimizer=tf.keras.optimizers.Adam(learning_rate=lr), 
            loss=spatial_aware_loss(
                self.num_classes, class_weights=weights,
                focal_weight=5.0, dice_weight=6.0, area_weight=2.0, 
                entropy_weight=0.05, tv_weight=5e-6
            ),
