...
class DataPipeline:
    ...
    def _scan_classes(self):
        ...
        classes = set()
        for j_path in list(self.train_dir.glob("*.json")) + list(
            self.val_dir.glob("*.json")
        ):
                            ...
                            if label in self.class_names:
                                ...
                                try:
                                    ...
                                    # 3. Обработка Полигонов
                                        ...
                                        cv2.fillPoly(
                                            temp, [np.array(pts, np.int32)], class_idx
                                        )
                                        mask = np.where(temp > 0, class_idx, mask)

                                except Exception as e:
                                    self.logger.warning(
                                        f"Error drawing shape {label} in {img_path.name}: {e}"
                                    )
    ...
    def create_datasets(self):
        ...
        output_signature = (
            tf.TensorSpec(
                shape=(self.img_size[1], self.img_size[0], 3), dtype=tf.float32
            ),
            tf.TensorSpec(
                shape=(self.img_size[1], self.img_size[0], 1), dtype=tf.int32
            ),
        )
        ...
        train_ds = (
            train_ds.shuffle(100).batch(self.batch_size).prefetch(tf.data.AUTOTUNE)
        )
