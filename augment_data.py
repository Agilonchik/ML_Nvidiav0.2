...
def decode_mask(b64_str):
    try:
        ...
        mask_img = cv2.imdecode(
            np.frombuffer(mask_bytes, np.uint8), cv2.IMREAD_UNCHANGED
        )
    ...
    if shape.get("shape_type") == "mask" and "mask" in shape:
        ...
        if m is not None:
            ...
            if (ye - my_start) > 0 and (xe - mx_start) > 0:
                full_m[my_start:ye, mx_start:xe] = m[
                    my_start - my : ye - my, mx_start - mx : xe - mx
                ]
    return full_m
...
def extract_local_mask(full_m):
    contours, _ = cv2.findContours(
        (full_m > 0).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    if not contours:
        return None, None
...
def augment_crop(img, data):
    ...
    x1, y1 = (
        int(max(0, min(tp[0] - cw // 2, w - cw))),
        int(max(0, min(tp[1] - ch // 2, h - ch))),
    )
    x2, y2 = x1 + cw, y1 + ch
    ...
    for s in new_data.get("shapes", []):
        if s.get("shape_type") == "mask" and "mask" in s:
            ...
            np_pts = (np.array(s["points"]) - [x1, y1]) * [
                TARGET_SIZE[0] / cw,
                TARGET_SIZE[1] / ch,
            ]
            if np.any(
                (np_pts[:, 0] >= 0)
                & (np_pts[:, 0] <= TARGET_SIZE[0])
                & (np_pts[:, 1] >= 0)
                & (np_pts[:, 1] <= TARGET_SIZE[1])
            ):
...
def augment_color(img, data):
    new_img = cv2.convertScaleAbs(
        img, alpha=random.uniform(0.8, 1.2), beta=random.randint(-30, 30)
    )
...
def main():
    ...
    mods = []
    if "1" in choice:
        mods.append((augment_flip, "aug_flip", "Отражение"))
    if "2" in choice:
        mods.append((augment_color, "aug_color", "Цвет"))
    if "3" in choice:
        mods.append((augment_crop, "aug_crop", "Обрезка"))
    if "4" in choice:
        mods.append((augment_noise_blur, "aug_noise", "Шум/Размытие"))
    if "5" in choice:
        mods.append((augment_cutout, "aug_cutout", "Перекрытия"))
    ...
    for current_dir in target_dirs:
        ...
        for m_func, sfx_name, ru_name in mods:
            ...
            print(
                f"   Файлов в очереди на обработку: {len(files_to_process)}", flush=True
            )
    ...
    print("\n" + "=" * 50, flush=True)
    print(
        f"🎉 СЕССИЯ ЗАВЕРШЕНА! Всего сгенерировано новых файлов: {total_generated}",
        flush=True,
    )
    print("=" * 50 + "\n", flush=True)
